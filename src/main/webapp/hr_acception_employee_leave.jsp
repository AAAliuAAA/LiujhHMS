...
<script type="text/x-jquery-tmpl" id="hr_leave_tab_template">
        <tr>
            ...
            <td>
                <div class="btn-group">
                    <a class="btn btn-primary" href="javascript:hrPass(${'${'}leaveId})"><i class="icon_plus_alt2"></i>&nbsp;通过</a>
                    <a class="btn btn-danger" href="javascript:hrRejection(${'${'}leaveId})"><i class="icon_close_alt2"></i>&nbsp;打回</a>
                </div>
            </td>
     </tr>
</script>
<script type="text/javascript">
  ...
  function pageInit(pageNo) {
    ...
    $.getJSON(url, data, function (data) {
      ...
      if (data.list.length == 0 || data.list == null) {
        $("#leave_hr_tab").append("<tr><td colspan=\"8\">暂时没有需要处理的请假申请</td></tr>");
        return;
      }
  ...
  function hrPass(leaveId) {
    //hr通过请假申请
    var url = "/leave/hr/pass.do";
    $.ajax({
      type: "POST",
      url: url,
      data: {
      ...
      success: function (data) {
        if (data == true) {
          messageAlert("请假申请通过成功");
        } else {
          messageAlert("请假申请通过失败");
        }
  ...
  function hrRejection(leaveId) {
    var url = "/leave/hr/rejection.do";
    $.ajax({
      type: "POST",
      url: url,
      data: {
      ...
      success: function (data) {
        if (data == true) {
          messageAlert("请假申请打回成功");
        } else {
          messageAlert("请假申请打回失败");
        }
        pageInit(parseInt($("#pageNum").text() + ""));
      }
    })
